...
import copy
from src.model.custom.layer_custom import Diagonal
...
LOG_PATH = join(os.getcwd(), 'logs')
REACTOM_PATHWAY_PATH = join(os.getcwd(), 'data/pathways/Reactome')
PROTEIN_PATH = join(os.getcwd(), 'data/protein')
layer_names = ['inputs']
# = ['inputs', 'h0', 'h1', 'h2', 'h3', 'h4', 'h5', 'l6']


# Don't residual connectivity
class SubModel(nn.Module):
    ...
    def __init__(self, model):
        ...
        for i in range(len(model.hidden_layers)):
            self.__setattr__(layer_names[i + 2], copy.deepcopy(model.hidden_layers[i]))
        self.__setattr__(layer_names[-1], model.linears[-1])

    def forward(self, input):
        out = tanh(self.h0(input))
        out = self.dropout1(out)

        for i in range(2, len(layer_names) - 1):
            out = tanh(self.__getattr__(layer_names[i])(out))
            if i != len(layer_names) - 2:
                out = self.dropout2(out)
        
        out = self.__getattr__(layer_names[-1])(out)
        return out


def save_gradient_importance(node_weights_):
    if not exists(join(RESULT_PATH, 'extracted')):
        os.makedirs(join(RESULT_PATH, 'extracted'))
...
def get_reactome_pathway_names():
    ...
    protein_df = pd.read_csv(join(PROTEIN_PATH, '9606.protein.info.v12.0.txt'), sep='\t')
    protein_df.rename(columns={
            '#string_protein_id': 'id', 'annotation': 'name'
        }, 
        inplace=True
    )
    protein_df['name'] = protein_df['name'].apply(lambda x: x.split(';')[0])
    reactome_pathways_df_human = pd.concat(
        [reactome_pathways_df_human[['id', 'name']], protein_df[['id', 'name']]],
        ignore_index=True
    )
...
def interpret(config, model, dataloader, local_model=False): #X=None, method_name='deeplift', baseline='zero'):
    global RESULT_PATH
    logger = getLogger()
    if config['interpretability'] == False:
        logger.info("according config file, don't interpret model!")
        return
    ...
    if local_model == True:
        ...
        method_name = config['method_name'] if config['method_name'] else 'deeplift'
        baseline = config['baseline'] if config['baseline'] else 'baseline'
        X = dataloader.x_test_
        RESULT_PATH = config['output_dir']
        model.to('cpu')
    global layer_names
    for i in range(len(model.hidden_layers) + 1):
         layer_names.append(f'h{i}')
    layer_names.append(f'l{len(model.hidden_layers) + 1}')
    ...
    # get link weights
    link_weights_df = get_link_weights_df_(sub_model, model.feature_names, layer_names)
    print("saving link weights")
    save_link_weights(link_weights_df, layer_names[1:])

    # get degree of genes
    deg_matrix = get_degrees(link_weights_df, layer_names[1:], sub_model)
    ...
    sankey.run(RESULT_PATH, config['n_hidden_layers'] + 2)


if __name__ == "__main__":
    interpret(model_name='pnet_deeplift', method_name='integratedgradients')