...
breast_data_path = join(data_path, 'breast')
processed_path = join(breast_data_path, 'processed')

# use this one
gene_final_no_silent_no_intron = 'P1000_final_analysis_set_cross__no_silent_no_introns_not_from_the_paper.csv'
# cnv_filename = 'data_cna_merge.csv'
# response_filename = 'response.csv'
# gene_important_mutations_only = 'mutation_matrix.csv'
cnv_filename = 'data_CNA_paper.csv'
response_filename = 'response_paper.csv'
gene_important_mutations_only = 'final_analysis_set_cross_important_only.csv'
...
def set_cached_data_to_empty():
    global cached_data
    cached_data = {}

def load_data(filename, selected_genes=None):
    filename = join(processed_path, filename)
    print('loading data_access from %s,' %(filename))
    if filename in cached_data:
        print('loading from memory cached_data')
    ...
    # zero_cols = data_access.sum(axis=0) == 0
    # data_access = data_access.loc[:, ~zero_cols]
    ...
    if not selected_genes is None:
        ...
        if len(intersect) < len(selected_genes):
            ...
            print('some genes dont exist in the original data_access set')
        x = x.loc[:, intersect]
        genes = intersect
    print('loaded data_access %d samples, %d variables, %d responses ' % (x.shape[0], x.shape[1], response.shape[0]))
    print(len(genes))
    return x, response, samples, genes
    #x's shape is (1011, 8319), it is matrix, index is feature, colume is gennes;samples is feature name; response is number to feature; genes is genes only
...
def load_data_type(data_type='gene', cnv_levels=5, cnv_filter_single_event=True, mut_binary=False, selected_genes=None):
    ...
    if data_type == 'mut_no_silent_no_intron':
        ...
        if mut_binary:
            print('mut_binary = True')
            x[x > 1.] = 1.

    if data_type == 'mut_important':
        x, response, info, genes = load_data(gene_important_mutations_only, selected_genes) # put processed data_access about prostate genes into 14k genes
        if mut_binary:
            print('mut_binary = True')
            x[x > 1.] = 1.
...
def get_response():
    print('loading response from %s' % response_filename)
...
def combine(x_list, y_list, rows_list, cols_list, data_type_list, combine_type, use_coding_genes_only=False):
    ...
    # put genes on the first level and then the data_access type
    ...
    rows = all_data.index
    # print(
    #     'After combining, loaded data_access %d samples, %d variables, %d responses ' % (x.shape[0], x.shape[1], y.shape[0]))
...
class BnetData():
    ...
    def __init__(self, config:Config):
        ...
        data_type = config['data_type'] if config['data_type'] else 'mut'

        account_for_data_type = config['account_for_data_type']

        cnv_levels = config['cnv_levels'] if config['cnv_levels'] else 5

        cnv_filter_single_event = config['cnv_filter_single_event'] \
            if config['cnv_filter_single_event'] else True
        
        mut_binary = config['mut_binary'] if config['mut_binary'] else False

        selected_genes = config['selected_genes']

        combine_type = config['combine_type'] \
            if config['combine_type'] else 'intersection'
        
        use_coding_genes_only = config['use_coding_genes_only'] \
            if config['use_coding_genes_only'] else False
        
        drop_AR = config['drop_AR'] if config['drop_AR'] else False

        balanced_data = config['balanced_data'] \
            if config['balanced_data'] else False
        
        cnv_split = config['cnv_split'] if config['cnv_split'] else False

        shuffle = config['shuffle'] if config['shuffle'] else False

        selected_samples = config['selected_samples']

        training_split = config['training_split'] \
            if config['training_split'] else 0
        ...
        if not selected_genes is None:
            if type(selected_genes) == list:
                ...
                selected_genes_file = join(data_path, 'genes') #data_path + genes name
                selected_genes_file = join(selected_genes_file, selected_genes) #genes path + experiment
                df = pd.read_csv(selected_genes_file, header=0) #put genes by storage in excel data_access into df
        ...
        if type(data_type) == list:
            ...
            x, y, rows, cols = load_data_type(data_type, cnv_levels, cnv_filter_single_event, mut_binary,
                                              selected_genes)
        self.logger.info(f'After combining, loaded data_access {x.shape[0]} samples, {x.shape[1]} variables, {y.shape[0]} responses.')
        if drop_AR:
    ...
    def get_train_validate_test(self):
        ...
        splits_path = join(breast_data_path, 'splits')
        ...
        print('Training set distribution:\n{}'.format(training_set['response'].value_counts()))

        info_train = list(sorted(set(info).intersection(training_set.id)))
        info_validate = list(sorted(set(info).intersection(validation_set.id)))
        info_test = list(sorted(set(info).intersection(testing_set.id)))
